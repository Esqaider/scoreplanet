<?php
   ...
   switch ($action) {
	    ...
	    case 'clear':
        $clear = $mysqli->query("DELETE FROM cart WHERE cart_ip = '{$_SERVER['REMOTE_ADDR']}'",$link);     
	    break;
        
        case 'delete':     
        $delete = $mysqli->query("DELETE FROM cart WHERE cart_id = '$id' AND cart_ip = '{$_SERVER['REMOTE_ADDR']}'",$link);        
        break;
...
if ( $_SESSION['auth'] == 'yes_auth' ) 
 {
    ...
    $mysqli->query("INSERT INTO orders(order_datetime,order_dostavka,order_fio,order_address,order_phone,order_note,order_email)
						VALUES(	
                             NOW(),
...
$result = $mysqli->query("SELECT * FROM cart WHERE cart_ip = '{$_SERVER['REMOTE_ADDR']}'",$link);
...
do{
    ...
    $mysqli->query("INSERT INTO buy_products(buy_id_order,buy_id_product,buy_count_product)
						VALUES(	
                            '".$_SESSION["order_id"]."',					
							'".$row["cart_id_product"]."',
                            '".$row["cart_count"]."'                   
...
$result = $mysqli->query("SELECT * FROM cart,table_products WHERE cart.cart_ip = '{$_SERVER['REMOTE_ADDR']}' AND table_products.products_id = cart.cart_id_product",$link);
If (mysqli_num_rows($result) > 0)
{
$row = mysqli_fetch_array($result);
